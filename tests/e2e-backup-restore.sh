#!/bin/bash
# End-to-end tests for the mailu-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-mailu}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-mailu-traefik-letsencrypt-docker-compose.yml}"
...
BACKUPS_PATH="${MAILU_BACKUPS_PATH%/}"
DB_PREFIX="${MAILU_BACKUP_NAME}"
DATA_PREFIX="${MAILU_DATA_BACKUP_NAME}"
INTERVAL="${MAILU_BACKUP_INTERVAL}"
DB_FILE="main.db"
DB_BASE="main"
...
test_restore_roundtrip() {
  ...
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./mailu-restore-data.sh "$stamp" > /dev/null \
    || { fail "./mailu-restore-data.sh failed"; return 1; }
  local exists
...
echo "=== mailu: backup/restore E2E tests ==="
