...
[![OpenSSF Best Practices](https://www.bestpractices.dev/projects/14840/badge)](https://www.bestpractices.dev/projects/14840)
...
Every release from v1.7.7 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `mailu-traefik-letsencrypt-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `mailu-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `mailu-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob mailu-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --bundle mailu-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/mailu-traefik-letsencrypt-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact mailu-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --provenance-path mailu-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/mailu-traefik-letsencrypt-docker-compose
```
